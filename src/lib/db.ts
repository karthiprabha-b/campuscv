...
// Seed default starter coupons if table is empty
try {
  const existingCoupons = couponDb.getAllCoupons();
  if (existingCoupons.length === 0) {
    const starterCoupons: Omit<DbCoupon, 'id'>[] = [
      {
        code: 'LAUNCH50',
    ...
    starterCoupons.forEach(c => couponDb.upsertCoupon(c));
    console.log('[COUPON_DB] Seeded initial promotional coupons into SQLite');
  }
} catch (e) {
  console.warn('[COUPON_DB] Could not seed starter coupons:', e);
}
