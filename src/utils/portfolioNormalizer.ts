...
export function normalizePortfolio(raw: any): PortfolioData {
  ...
  if ((!sectionFiles || Object.keys(sectionFiles).length === 0) && (raw.templateId || raw.layoutStyle)) {
    ...
    const normalizedObj: PortfolioData = {
      ...boundProps,
      ...raw,
      name: raw.name || raw.fullName || boundProps.name || canonicalProfile.personal?.fullName || '',
      fullName: raw.fullName || raw.name || boundProps.fullName || canonicalProfile.personal?.fullName || '',
      headline: raw.headline || raw.role || raw.tagline || boundProps.headline || canonicalProfile.personal?.headline || '',
      role: raw.role || raw.headline || raw.tagline || boundProps.role || canonicalProfile.personal?.headline || '',
      location: resolvedLocation,
      profileImage: resolvedImg || raw.profileImage || boundProps.profileImage,
      avatarUrl: resolvedImg || raw.avatarUrl || boundProps.avatarUrl,
      avatar: resolvedImg,
      photo: resolvedImg,
      image: resolvedImg,
      email: emailVal,
      ...
      hero: {
        ...(boundProps.hero || {}),
        ...(raw.hero || {}),
        name: raw.hero?.name || raw.name || raw.fullName || boundProps.hero?.name || boundProps.name || canonicalProfile.personal?.fullName || '',
        role: raw.hero?.role || raw.role || raw.headline || boundProps.hero?.role || boundProps.role || canonicalProfile.personal?.headline || '',
        title: raw.hero?.title || raw.hero?.headline || raw.heroHeadline || raw.headline || raw.title || raw.tagline || boundProps.hero?.title || boundProps.headline || canonicalProfile.personal?.headline || '',
        headline: raw.hero?.headline || raw.hero?.title || raw.heroHeadline || raw.headline || raw.title || raw.tagline || boundProps.hero?.headline || boundProps.headline || canonicalProfile.personal?.headline || '',
        subtitle: raw.hero?.subtitle || raw.hero?.description || raw.heroDescription || raw.aboutMe || raw.summary || raw.bio || boundProps.hero?.subtitle || boundProps.headline || canonicalProfile.personal?.headline || '',
        description: raw.hero?.description || raw.hero?.subtitle || raw.heroDescription || raw.aboutMe || raw.summary || raw.bio || boundProps.hero?.description || cleanSummary,
        introductionText: raw.hero?.introductionText || raw.hero?.description || raw.aboutMe || cleanSummary,
      ...
      about: {
        ...(boundProps.about || {}),
        ...(raw.about || {}),
        description: raw.about?.description || raw.about?.bio || raw.aboutMe || raw.bio || raw.summary || boundProps.about?.description || cleanSummary,
        bio: raw.about?.bio || raw.about?.description || raw.aboutMe || raw.bio || raw.summary || boundProps.about?.bio || cleanSummary,
