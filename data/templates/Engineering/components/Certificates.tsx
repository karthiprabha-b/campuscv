...
export default function Certificates() {
  ...
  return (
    <section id="certificates" className="py-16 sm:py-24 md:py-32 px-4 sm:px-6 lg:px-8 bg-gradient-indigo-card text-white wave-top-curve wave-bottom-curve relative overflow-hidden shadow-2xl">
      {/* Background Glow */}
            ...
            >
              <div>
                ...
                <div className="flex items-center justify-between gap-2 pb-3 mb-3 border-b border-slate-100">
                  <span className="text-xs font-bold font-mono text-cyber-600 uppercase tracking-wider">
                    {cert.issuer}
                  </span>
                ...
                {/* Name */}
                <h3 className="font-display font-black text-base sm:text-xl text-slate-950 leading-snug mb-3">
                  {cert.name}
                </h3>

                {/* Credential ID */}
                <div className="flex items-center gap-2 text-xs font-mono text-slate-600 bg-slate-50 px-3.5 py-1.5 rounded-full mb-4">
                  <KeyRound className="w-3.5 h-3.5 text-cyber-600 shrink-0" />
                ...
                <div className="flex flex-wrap gap-1.5 mb-6">
                  {cert.skillsCovered.map((skill, sIdx) => (
                    <span
                      key={sIdx}
                      className="px-2.5 py-0.5 bg-slate-100 rounded-full text-[10px] sm:text-[11px] font-mono text-slate-700"
                    >
                      {skill}
                    </span>
                  ))}
