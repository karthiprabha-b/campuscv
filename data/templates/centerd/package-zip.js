...
async function buildTemplateZip() {
  ...
  const zip = new JSZip();
  const outputPath = path.join(rootDir, 'centerd-template.zip');
  ...
  const rootZip1 = path.join(rootDir, '..', '..', 'centerd.zip');
  const rootZip2 = path.join(rootDir, '..', '..', 'centerd-template.zip');
  fs.writeFileSync(rootZip1, buf);
  fs.writeFileSync(rootZip2, buf);

  console.log(`\x1b[32m✔ Successfully created centerd-template.zip (${(buf.length / 1024).toFixed(1)} KB)\x1b[0m`);
}
