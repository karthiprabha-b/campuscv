"use client";

import React from "react";
import { Briefcase, Calendar } from "lucide-react";
import { ExperienceItem } from "@/data/portfolio";

interface ExperienceProps {
  data?: any;
  experiences?: any[];
  experience?: any[];
}

export default function Experience(props: ExperienceProps = {}) {
  const rawList = (Array.isArray(props.experiences) && props.experiences.length > 0)
    ? props.experiences
    : (Array.isArray(props.experience) && props.experience.length > 0)
      ? props.experience
      : (Array.isArray(props.data?.experiences) && props.data.experiences.length > 0)
        ? props.data.experiences
        : (Array.isArray(props.data?.experience) && props.data.experience.length > 0)
          ? props.data.experience
          : (Array.isArray(props.data?.canonicalProfile?.experience) && props.data.canonicalProfile.experience.length > 0)
            ? props.data.canonicalProfile.experience
            : (Array.isArray(props.data?.workExperience) && props.data.workExperience.length > 0)
              ? props.data.workExperience
              : (Array.isArray(props.data?.work) && props.data.work.length > 0)
                ? props.data.work
                : (Array.isArray(props.data?.timeline) && props.data.timeline.length > 0)
                  ? props.data.timeline
                  : (Array.isArray(props.data?.resume?.experience) && props.data.resume.experience.length > 0)
                    ? props.data.resume.experience
                    : [];

  if (!rawList || !Array.isArray(rawList) || rawList.length === 0) {
    return null;
  }

  // Filter out demo experience if real experience exists
  const isDemoExp = (e: any) => {
    const c = (e.company || '').toLowerCase();
    const r = (e.role || '').toLowerCase();
    return c.includes('creative spark studio') || (c.includes('pixel perfect') && r.includes('junior web developer'));
  };

  const hasReal = rawList.some((e: any) => !isDemoExp(e));
  const candidateList = hasReal ? rawList.filter((e: any) => !isDemoExp(e)) : rawList;

  const experiencesList: ExperienceItem[] = candidateList.map((e: any) => {
    const rawDesc = e.description || e.desc || e.summary;
    let bullets: string[] = [];
    if (Array.isArray(rawDesc)) {
      bullets = rawDesc;
    } else if (Array.isArray(e.bullets)) {
      bullets = e.bullets;
    } else if (Array.isArray(e.highlights)) {
      bullets = e.highlights;
    } else if (typeof rawDesc === "string" && rawDesc.trim()) {
      bullets = rawDesc.split("\n").map((s: string) => s.replace(/^[-*•]\s*/, "").trim()).filter(Boolean);
    }

    if (bullets.length === 0 && (e.role || e.title || e.subtitle)) {
      bullets = [e.role || e.title || e.subtitle];
    }

    const start = (e.startDate || e.startYear || '').toString().trim();
    const end = (e.endDate || e.endYear || '').toString().trim();
    const period = (e.duration || e.period || (start && end ? `${start} — ${end}` : (start || end || ''))).toString().trim();

    return {
      company: e.company || e.organization || e.client || e.subtitle || "Organization",
      role: e.role || e.position || e.title || "Role / Position",
      duration: period,
      description: bullets
    };
  });

  if (experiencesList.length === 0) {
    return null;
  }

  return (
    <section
      id="experience"
      data-section="experience"
      data-cv-section="experience"
      className="py-24 px-6 sm:px-12 md:px-16 lg:px-24 border-t border-[#111111]/10 relative z-10 bg-[#FAF9F6]"
    >
      <div className="max-w-4xl mx-auto">
        {/* Section Heading */}
        <div className="flex flex-col mb-16 items-center text-center space-y-3">
          <span className="text-xs font-black tracking-widest text-[#FFC107] uppercase">
            Career
          </span>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-wide leading-tight text-[#111111]">
            WORK EXPERIENCE
          </h2>
          <div className="w-12 h-1 bg-[#FFC107] mt-2" style={{ backgroundColor: "#FFC107" }} />
        </div>

        {/* Timeline Cards Container */}
        <div className="relative border-l-2 border-[#111111]/10 pl-6 sm:pl-8 space-y-12">
          {experiencesList.map((exp: ExperienceItem, idx: number) => {
            return (
              <div
                key={exp.company || idx}
                data-cv={`experience[${idx}]`}
                data-cv-item
                data-cv-index={idx}
                className="relative group"
              >
                {/* Timeline Bullet Node */}
                <div className="absolute left-[-33px] sm:left-[-41px] top-1 z-10 p-1.5 sm:p-2 rounded-full bg-[#111111] text-[#FAF9F6] border-4 border-[#FAF9F6] group-hover:bg-[#FFC107] group-hover:text-[#111111] transition-all duration-300 shadow-sm">
                  <Briefcase className="w-3.5 h-3.5 sm:w-4.5 sm:h-4.5 stroke-[2]" />
                </div>

                {/* Card Container */}
                <div className="p-6 bg-white border border-[#111111]/10 rounded-md hover:border-[#FFC107]/60 transition-colors duration-300 shadow-sm">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
                    <div>
                      <span className="text-[10px] font-black text-[#FFC107] tracking-wider uppercase">
                        {exp.company}
                      </span>
                      <h3 className="text-base sm:text-lg font-black tracking-tight text-[#111111] mt-0.5">
                        {exp.role}
                      </h3>
                    </div>

                    {/* Year Tag */}
                    {exp.duration && (
                      <div className="inline-flex items-center text-[10px] sm:text-xs font-bold bg-[#111111]/5 px-2.5 py-1 text-[#666666] rounded-full w-fit">
                        <Calendar className="w-3 h-3 mr-1 opacity-70" />
                        {exp.duration}
                      </div>
                    )}
                  </div>

                  {/* Bullets List */}
                  {exp.description.length > 0 && (
                    <ul className="space-y-2 text-xs sm:text-sm text-[#666666] list-disc pl-4 leading-relaxed">
                      {exp.description.map((bullet: string, bIdx: number) => (
                        <li key={bIdx} className="marker:text-[#FFC107]">
                          {bullet}
                        </li>
                      ))}
                    </ul>
                  )}
                </div>
              </div>
            );
          })}
        </div>
      </div>
    </section>
  );
}
