...
const sourceDir = path.resolve(__dirname, '../data/templates/Beautician');
const zipFile = path.resolve(__dirname, '../data/templates/Beautician.zip');
...
const psPath = path.resolve(__dirname, 'pack_beautician.ps1');
...
try {
  ...
  fs.copyFileSync(zipFile, path.join(publicDir, 'beautician.zip'));
  fs.copyFileSync(zipFile, path.join(publicDir, 'beautician-template.zip'));
  fs.copyFileSync(zipFile, path.join(publicDir, 'beautician-portfolio.zip'));
  fs.copyFileSync(zipFile, path.join(publicDir, 'Beautician.zip'));
